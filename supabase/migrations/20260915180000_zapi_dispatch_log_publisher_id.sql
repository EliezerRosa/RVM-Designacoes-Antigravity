-- Migration: 20260915180000_zapi_dispatch_log_publisher_id.sql
-- Descrição: Adiciona coluna publisher_id e atualiza a view canônica para usá-la se wp.resolved_publisher_id falhar.

ALTER TABLE public.zapi_dispatch_log ADD COLUMN IF NOT EXISTS publisher_id text;
...
SELECT 
    ...
    COALESCE(
        pub.data->>'name', 
        (SELECT p.data->>'name' FROM public.publishers p WHERE p.id::text = z.publisher_id LIMIT 1),
        (SELECT p.data->>'name' FROM public.publishers p WHERE p.data->>'phone' = z.recipient_phone LIMIT 1),
        'Desconhecido'
    ) AS recipient_name,
    COALESCE(pub.id::text, z.publisher_id) AS publisher_id,
    z.part_id,
