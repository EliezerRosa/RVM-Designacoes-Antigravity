...
-- Atualização da RPC get_pending_push_events para incluir o token de confirmação
-- ============================================================================
...
BEGIN
    ...
    -- LEFT JOIN com tokens válidos para obter o link mágico, se existir
    LEFT JOIN LATERAL (
        SELECT t.token
        FROM public.confirmation_portal_tokens t
        WHERE t.part_id = wp.id::text
          AND t.publisher_id = pub.id::text
    ...
    WHERE 
        -- Apenas eventos das últimas 48h
