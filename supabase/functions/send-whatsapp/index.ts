...
/** Verifica se a string parece ser apenas um número de telefone mascarado em vez de um nome real. */
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const origin = req.headers.get('Origin') || '';
    const isBrowserRequest = origin || req.headers.has('Authorization');

    // ── [SECURITY] Server-side 2FA OTP request ──
    // Moved UP so non-editors can access it (JWT is validated inside the block)
    if (body.action === 'request_2fa') {
      ...
      const serverCode = rpcResult.code;
      if (!serverCode) {
        return new Response(
          JSON.stringify({ success: false, error: 'internal_no_code' }),
          { status: 500, headers: { ...getCorsHeaders(req), 'Content-Type': 'application/json' } }
        );
      }
      ...
      if (provider === 'meta-cloud') {
        sendRes = await sendViaMeta(twoFaPhone, msg);
      } else if (provider === 'z-api') {
        sendRes = await sendViaZApi(twoFaPhone, msg);
      } else {
        sendRes = await sendViaEvolution(twoFaPhone, msg);
      }

      return new Response(
        JSON.stringify({ success: true, warning: sendRes.success ? undefined : 'send_failed' }),
    ...
    // [SECURITY] Se não for editor, checar se target é publicador verificado
    if (!isEditor) {
