...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ── [SECURITY] Server-side 2FA OTP request ──
    // The OTP code is generated by the DB RPC, sent via WhatsApp here,
    // and NEVER returned to the browser. The client only sees { success: true }.
    if (body.action === 'request_2fa') {
      const twoFaPhone = (body.phone || '').trim();
      ...
      // Verify caller has a valid JWT (any authenticated user, not just editor)
      const authHeader = req.headers.get('Authorization');
      if (!authHeader) {
        return new Response(
      ...
      // Use the user's own JWT to call the RPC (rate-limit + auth.uid() work natively)
      const userClient = createClient(supabaseUrl, supabaseAnonKey, {
        auth: { persistSession: false },
        global: { headers: { Authorization: `Bearer ${jwt}` } },
      });

      const { data: rpcData, error: rpcError } = await userClient.rpc('create_whatsapp_auth_request', {
        p_phone: twoFaPhone,
      });
      ...
      if (rpcError) {
        ...
        return new Response(
          JSON.stringify({ success: false, error: 'rpc_failed' }),
          { status: 500, headers: { ...getCorsHeaders(req), 'Content-Type': 'application/json' } }
        );
      }

      const rpcResult = rpcData as { success?: boolean; error?: string; code?: string; message?: string } || {};
      if (!rpcResult.success) {
        // Forward rate_limited / phone_already_in_use errors to client (without the code)
        return new Response(
          JSON.stringify({ success: false, error: rpcResult.error || 'unknown', message: rpcResult.message }),
          { status: 429, headers: { ...getCorsHeaders(req), 'Content-Type': 'application/json' } }
        );
      }

      // The code exists only server-side here — send it via WhatsApp
      const otpCode = rpcResult.code;
      if (!otpCode) {
        console.error('[2FA] RPC returned success but no code');
        return new Response(
          JSON.stringify({ success: false, error: 'no_code_generated' }),
          { status: 500, headers: { ...getCorsHeaders(req), 'Content-Type': 'application/json' } }
        );
      }

      const otpMessage = `*RVM Designações*\n\nSeu código de acesso é: *${otpCode}*\n\n_Não compartilhe este código com ninguém._`;
      const normalizedOtpPhone = normalizePhone(twoFaPhone);

      let sendResult;
      if (provider === 'meta-cloud') {
        sendResult = await sendViaMeta(normalizedOtpPhone, otpMessage);
      } else if (provider === 'z-api') {
        sendResult = await sendViaZApi(normalizedOtpPhone, otpMessage);
      } else {
        sendResult = await sendViaEvolution(normalizedOtpPhone, otpMessage);
      }

      console.log(`[2FA] OTP sent to ${normalizedOtpPhone} via ${provider}:`, sendResult.success);

      // Return success to client WITHOUT the code
      return new Response(
        JSON.stringify({
          success: true,
          delivered: sendResult.success,
          // If sending failed, tell the client so it can show a fallback message
          ...(sendResult.success ? {} : { warning: 'send_failed', detail: sendResult.error }),
        }),
        { headers: { ...getCorsHeaders(req), 'Content-Type': 'application/json' } }
      );
    }
