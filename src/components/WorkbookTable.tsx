...
function StatusBadge({ part, eventNotesMap = {} }: { part: WorkbookPart, eventNotesMap?: Record<string, string> }) {
    ...
    const eventNote = part.affectedByEventId ? eventNotesMap[part.affectedByEventId] : undefined;
    const hasCustomNote = !!eventNote;
    ...
    const badge = (
        <span style={{
            ...
            gap: '4px',
            fontWeight: '600',
            cursor: (isCancelled && part.cancelReason) || hasCustomNote ? 'help' : 'default',
            ...animationStyle,
        }}>
            {eventIcon && !hasCustomNote && <span title={eventTitle}>{eventIcon}</span>}
            {hasCustomNote && <span>📌</span>}
        ...
        return (
            <Tooltip content={
                <div style={{ padding: '4px' }}>
                    ...
                    {hasCustomNote && (
                        <div style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px solid rgba(255,255,255,0.2)', fontSize: '11px', fontStyle: 'italic' }}>
                            Nota S-140: {eventNote}
                        </div>
                    )}
                </div>
            }>
    ...
    if (hasCustomNote && !isCancelled) {
        return (
            <Tooltip content={
                <div style={{ padding: '4px' }}>
                    <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>📌 Nota Híbrida do S-140</div>
                    <div style={{ fontSize: '12px', fontStyle: 'italic' }}>"{eventNote}"</div>
                </div>
            }>
                {badge}
            </Tooltip>
